        ...
        if datetime.datetime.now().hour in  [morning, changetime]:
            ...
            if send_or_not == 'False':
                ...
                if morning != datetime.datetime.now().hour:
                    ...
                    # 发送假数据
                    s.send_mail_out('139IOS客户端'+test_version+'版本_功能拨测_24小时汇总', false_txt,is_test=is_test)
                    time.sleep(10)
                    # 发送真数据
                    s.send_mail('【内部邮件】139IOS客户端'+test_version+'版本_功能拨测_24小时汇总', all_sendtxt,is_test=is_test)
                else:
                    # 发送假数据
                    s.send_mail_out('139IOS客户端'+test_version+'版本_功能拨测_晚上部分汇总', false_txt,is_test=is_test)
                    time.sleep(10)
                    # 发送真数据
                    s.send_mail('【内部邮件】IOS客户端'+test_version+'版本_功能拨测_晚上部分汇总', all_sendtxt,is_test=is_test)
            ...
            if len(err) != 0:
                ...
                s = SendMail("13580491603","chinasoft123","13697485262")
                s.send_mail_str('139IOS客户端'+test_version+'版本_功能拨测_出现错误<内部邮件>',errstr,is_test=is_test)
